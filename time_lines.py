...
class TimeLines:
    ...
    def sort_tl_events(self):
        def jd_interval_sorter(jds):
            if len(jds) == 1:
                ...
                ls = jds
            return (ls[0], ls[1])
        self.tl_events = sorted(self.tl_events, key=lambda x: jd_interval_sorter(x[0]))
    ...
    def add_date_table_events(self, table, tables_unique_domain_names, check_order=True):
        event_cnt = 0
        events_skipped = 0
        ...
        if table["metadata"]["domain"] in tables_unique_domain_names:
            ...
            return event_cnt, 0
        tables_unique_domain_names.append(table["metadata"]["domain"])
    ...
    def add_notes_events(self, notes):
        event_cnt = 0
        skipped_cnt = 0
        tables_unique_domain_names = []
    ...
    def notes_rest(self, do_timeline, format, timespec, domains, keywords):
